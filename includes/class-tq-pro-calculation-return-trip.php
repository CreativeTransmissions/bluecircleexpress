...
class TQ_CalculationReturnTrip {
	...
	public function init_calculation(){
		...
		$this->outward_distance = self::get_outward_distance();
	}
	...
	public function calc_for_outward_distance(){
		$this->distance = $this->outward_distance;
		switch ($this->config['boundary_mode']) {
			case 'final':
				self::calc_with_final_boundary_rates();
				break;
...
*/
	...
	/*	echo ', add_return_distance_cost_to_distance_cost: rate: '.$rate['unit'];
		...
		//echo ', add_max_distance_cost_to_distance_cost: miles_remaining: '.$miles_remaining;
		$this->basic_return_cost = $return_cost = self::calculate_return_cost($rate);
		...
		$this->units_charged_for = $this->units_charged_for + $this->return_miles;
		$this->breakdown[] = array(	'distance'=>$this->return_miles,
									'distance_cost'=>$this->distance_cost,
									'type'=>'per distance unit',
									'rate'=>$rate['unit'],
									'return_percentage'=>$this->config['return_percentage'],
									'basic_return_cost'=>$this->basic_return_cost,
									'adjusted_return_cost'=>$this->return_cost,
									'cost'=>$return_cost);
	}

	public function calculate_return_cost($rate){
		$this->return_miles = $this->return_distance;
		$return_cost = $this->return_miles*$rate['unit'];
		return $return_cost;
	}
